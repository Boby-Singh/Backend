...
const payment_success = async(req, res) => {
    ...
    try {
        const sql = 'UPDATE orders SET status = ? WHERE session_id = ? AND user_id = ?';
        const [data] = await db.query(sql, ['success', paymentIntentId, userId]);
        res.status(200).json({ status: true, msg: 'Status has been updated successfully', data });
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Internal Server Error' });
    }
